...
it('affiche la page de facturation avec le plan actuel et les 6 plans disponibles', function (): void {
...
it('échoue proprement au moment du paiement quand le Price ID Stripe du plan n\'est pas configuré', function (): void {
    config(['plans.personal_essential.stripe_price' => null]);
    ['doorStaff' => $owner] = makeCheckInEvent(MembershipRole::Owner);

    $response = $this->actingAs($owner)->post('/billing/checkout/personal_essential');
...
it('refuse un changement de plan sans abonnement Stripe actif', function (): void {
    ...
    $this->actingAs($owner)->post('/billing/change/professional_career')
        ->assertStatus(500);
});
